...
export const createCommentProvider = async (req, res) => {
  try {
    ...
    if (!text && !img) {
      return res.status(400).json({ error: "no content is given" });
    }
...
export const likeUnlikePOstProvider = async (req, res) => {
  try {
    ...
    console.log(`Error occured in likeUnlikePOstProvider - ${error}`);
    res.status(500).json({ error: Error.message });
  }
};
...
export const getLikedPosts = async(req,res)=>{
    ...
    try{
        ...
        console.log(`Error in getLikedPostsprovider ${error}`);
...
export const getFollowingPosts = async(req,res)=>{
    try{
        ...
        console.log(`Error occured in getFollowingPosts controller ${error}`);
        res.status(500).json({error:error.message})
...
export const getUserPosts = async(req,res)=>{
    try{
        ...
        console.log(`Error occured in getUserPosts ${error}`);
        res.status(500).json({error:error.message})
    }
}